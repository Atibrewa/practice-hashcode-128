...
public class Student implements Comparable<Student> {
    ...
    @Override
    public int hashCode() {
        // f(Student object) = 17; Always returns the same hashcode, so every object will collide and occupy the same bucket
        // f(Student object) = name.hashcode(); will still have many collisions because there are only 26 possible codes
        // f(Student object) = studentID.hashcode(); is a good option as it gives us unique hashcodes every time, so no collisions, but it might have large/negative values

        // the best N value is the size of the array!
        return ((Integer)studentID).hashCode() % 11; // replace me
    }
}
